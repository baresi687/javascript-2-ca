...
sortPosts.onchange = function (event) {
  ...
  switch (currentValue) {
    ...
    case "title":
      if (sortDesc.checked) {
        getPosts(
          GET_POSTS_URL + sortTitleString,
          searchField.value.toLowerCase().trim()
        );
      } else {
        getPosts(
          GET_POSTS_URL + sortTitleString + sortAscString,
          searchField.value.toLowerCase().trim()
        );
      }
      break;
...
async function getPosts(url, searchValue = "") {
  ...
  try {
    ...
    const filterPosts = responseJSON.filter((elem, index, arr) => {
      const filterDuplicates = arr.findIndex(
        (dupElem) => dupElem.title === arr[index].title
      );
      return (
        filterDuplicates === index &&
        elem.title.length > 1 &&
        (elem.title.toLowerCase().includes(searchValue) ||
          elem.body.toLowerCase().includes(searchValue))
      );
    });
    let data;
